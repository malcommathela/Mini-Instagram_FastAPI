    ...
    if email and password:
        ...
        with col1:
            if st.button("Login", type="primary", use_container_width=True):
                ...
                response = requests.post("http://localhost:8080/auth/jwt/login", data=login_data)
                ...
                if response.status_code == 200:
                    ...
                    # Get user info
                    user_response = requests.get("http://localhost:8080/auth/jwt/me", headers=get_headers())
        ...
        with col2:
            if st.button("Sign Up", type="secondary", use_container_width=True):
                ...
                response = requests.post("http://localhost:8080/auth/register", json=signup_data)
...
def upload_page():
    ...
    if uploaded_file and st.button("Share", type="primary"):
        with st.spinner("Uploading..."):
            ...
            response = requests.post("http://localhost:8080/upload", files=files, data=data, headers=get_headers())
...
def feed_page():
    ...
    response = requests.get("http://localhost:8080/feed", headers=get_headers())
    if response.status_code == 200:
            ...
            with col2:
                if post.get('is_owner', False):
                    if st.button("🗑️", key=f"delete_{post['id']}", help="Delete post"):
                        # Delete the post
                        response = requests.delete(f"http://localhost:8080/posts/{post['id']}", headers=get_headers())
                        if response.status_code == 200:
